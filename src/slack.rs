...
use slack_morphism::{
    api::SlackApiChatPostMessageRequest, blocks::*, prelude::SlackClientHyperConnector,
    ...
    pub async fn log_message(
        ...
        let thread_ts = match THREAD_TS.get() {
            ...
            None => {
                ...
                THREAD_TS
                    .set(ts.clone())
                    .map_err(|_| anyhow::anyhow!("Failed to set thread timestamp"))?;
                ts
            }
        };
        ...
        blocks.push(SlackBlock::Section(SlackSectionBlock::new().with_text(
            SlackBlockText::MarkDown(SlackBlockMarkDownText::new(format!("*{}*", header_text))),
        )));

        // Add message content
        blocks.push(SlackBlock::Section(SlackSectionBlock::new().with_text(
            SlackBlockText::MarkDown(SlackBlockMarkDownText::new(message.to_string())),
        )));

        // Add context if provided
        if let Some(ctx) = context {
            blocks.push(SlackBlock::Context(SlackContextBlock::new(vec![
                SlackContextBlockElement::Plain(SlackBlockPlainText::new(format!(
                    "Context: {}",
                    ctx
                ))),
            ])));
        ...
        let post_req = SlackApiChatPostMessageRequest::new(self.channel_id.clone(), content)
    ...
    async fn create_thread(&self) -> Result<SlackTs> {
        ...
        let post_req = SlackApiChatPostMessageRequest::new(
            ...
            SlackMessageContent::new().with_text(format!(
                ":thread: *{}*\n_Starting new conversation log_",
                thread_title
            )),
        );
